...
const FreeLancherPrivacyPolicy = () => {
  ...
  // Load privacyPolicy data when component mounts
  useEffect(() => {
    if (policies?.data?.privacyPolicy) {
      setDescription(policies.data.privacyPolicy);
    }
  }, [policies]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        message.success(result.message || "Privacy policy saved successfully!");
      } else {
        message.error(result.message || "Failed to save privacy policy");
      }
    } catch (err) {
      console.error("Save error:", err);
      message.error(
        err?.data?.message || err?.message || "Failed to save privacy policy"
      );
    } finally {
      setIsSaving(false);
    }
